<template>
	<div>
		...
		<div class="container-fluid">
			<h4>Current Setup Available</h4>
		</div>
		<div class="container-fluid" style="max-height: 230px;overflow-y: auto;">
			<vuetable 
				ref="Vuetable_setup_list"
				api-url="/api/v1/setup/manpowerList"
				:fields="fields"
				pagination-path=""
				:css="css.table"
...
<script>
	...
	export default {
		...
		methods: {
			getVenues() {
                let url = `/api/v1/venues/plans/job-order/${$('#jobOrderNumber').val()}`;
                this.$http.get(url).then(response => {
