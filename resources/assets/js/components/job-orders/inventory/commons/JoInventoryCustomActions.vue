        ...
        methods: {
            itemAction (action, data, index) {
                ...
                if (action === 'delete-item') {
                    let url = `/api/v1/job-order-inventory/${data.id}`;
                    this.$http.delete(url, data).then(response => {
                        console.log(response)

                        toastr.success('Successfully deleted inventory request', 'Success')
                        this.$events.fire('reload-table')
                    }, error => {
                        toastr.success('Failed in deleting inventory request', 'Error')
                        console.log(error)
                    })
                }
